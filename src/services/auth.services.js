...
export const verifyUserToken = async (req, res) => {
  try {
    // El middleware verifyToken ya ha verificado el token y ha añadido req.user
...
export const verifyToken = (req, res, next) => {
  ...
  try {
    ...
    req.user = payload; // Guardamos la información del usuario en el request
    next();
  } catch (error) {
...
export const loginUser = async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      ...
      secretKey,
      { expiresIn: "24h" }
    ); // Aumentamos el tiempo de expiración a 24 horas
